...
const getBooks = async (req, res) => {
    ...
    const { title, subtitle, publicationYear, editionNumber, state } = req.query;
    const bookLanguage = req.query.language;

    const filterObject = { 
        $and: [
            { $or: [{ title: new RegExp(title === undefined || title === null ? constants.strings.EMPTY_STRING : title, 'i') }, { originalTitle: new RegExp(title === undefined || title === null ? constants.strings.EMPTY_STRING : title, 'i') }] }, 
            { $or: [{ subtitle: new RegExp(subtitle === undefined || subtitle === null ? constants.strings.EMPTY_STRING : subtitle, 'i') }, { originalSubtitle: new RegExp(subtitle === undefined || subtitle === null ? constants.strings.EMPTY_STRING : subtitle, 'i') }] }, 
            ...
            { state: state === undefined || state === null || state.length <= 0 || req.user.role !== constants.strings.ROLE_ADMIN ? constants.strings.STATE_OK : new RegExp(state, 'i') },  
        ]
    }; 
    ...
    // Get list of books.
    Book.find(filterObject).sort(constants.models.TITLE).limit(limit).skip((pageNumber - 1) * limit).exec((error, booksDB) => {
        if (error) {
